        ...
        <S.ModalBlock>
            ...
            <NavLink to = "/">
               <S.ModalLogo>
                ...
                <S.ModalInput type="text" name="login" id="loginReg"  autoComplete="username" placeholder="email" value={email} onChange={(event) => {
              setEmail(event.target.value)}}  /> 

            {errorsForm.password && <S.error>{errorsForm.password}</S.error>}
                <S.ModalInput type="password" name="password" id="passwordFirst" placeholder="Пароль" autoComplete="new-password" value={password} onInput={(event) => {
              setPassword(event.target.value)}}/>

            {errorsForm.confirmPassword && <S.error>{errorsForm.confirmPassword}</S.error>}
                <S.ModalInput type="password" name="password" id="passwordSecond" placeholder="Повторите пароль" autoComplete="new-password" value={confirmPassword}
            onChange={(event) => {setConfirmPassword(event.target.value)}}/>
                ...
                <S.ModalBtnSignupEnt id="SignUpEnter" onClick={handleReg} disabled={offButton}> {offButton ? "Осуществляем регистрацию" : "Зарегистрироваться"}</S.ModalBtnSignupEnt>
            </S.ModalFormLogin>
        </S.ModalBlock>
    </S.ContainerSignup>
</S.Wrapper>
