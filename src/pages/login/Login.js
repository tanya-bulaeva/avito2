    ...
    const handleLogin = async () => {
     if (validateForm()) {
          loginUser({email, password}).then(() => {
            ...
            navigate("/")).catch (error => setError(error?.message))
    };
}
return (
    <S.Wrapper>
    <S.ContainerEnter>
        <S.ModalBlock>
            <S.ModalFormLogin id="formLogIn" onSubmit={(e) => e.preventDefault()}>
                <NavLink to = "/">
                <S.ModalLogo>
                ...
                <S.ModalInputLogin type="text" name="login" id="formlogin" placeholder="email" value={email}
            onChange={(event) => {
              setEmail(event.target.value);
            }}/>

             {errorsForm.password && <S.error>{errorsForm.password}</S.error>}
                <S.ModalInput type="password" name="password" id="formpassword" placeholder="Пароль" autocomplete="current-password"    value={password}
            onChange={(event) => {
              setPassword(event.target.value);
            }}/>
            {error && <S.error>{error}</S.error>}
                <S.ModalBtnEnter id="btnEnter" onClick={ handleLogin } disabled={offButton}>   {offButton ? "Осуществляем вход" : "Войти"}
